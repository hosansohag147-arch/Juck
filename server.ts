...
// রেলওয়েতে (Railway) বসানো API Key স্ট্রিংটি নিয়ে অ্যারে বা লিস্ট বানানো
const rawKeys = process.env.GEMINI_API_KEYS || "";
const API_KEYS = rawKeys ? rawKeys.split(",").map(key => key.trim()) : [];
...
async function startServer() {
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    // যদি রেলওয়েতে কোনো কি সেট করা না থাকে
    if (API_KEYS.length === 0) {
      return res.status(500).json({ error: "Gemini API Keys are missing in Railway Environment Variables." });
    }
